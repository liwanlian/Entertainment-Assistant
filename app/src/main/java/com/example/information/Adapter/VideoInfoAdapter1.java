...
public class VideoInfoAdapter1 extends RecyclerView.Adapter<VideoInfoAdapter1.ViewHolder> {

    private List<Bean_video.Subjectdata> subjectdata;
    Context context;


    public VideoInfoAdapter1(List<Bean_video.Subjectdata> subjectdata) {
        this.subjectdata = subjectdata;
    }
    @NonNull
    @Override
    public VideoInfoAdapter1.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull  VideoInfoAdapter1.ViewHolder holder, int position) {

        holder.video_title.setText(subjectdata.get(position).getTitle()+"("+subjectdata.get(position).getOriginal_title()+")");
        String[] pubdate=subjectdata.get(position).getPubdates();
       holder.video_publishtime.setText("上映时间："+pubdate[0]);
        StringBuffer sb_director=new StringBuffer();
        for (int i=0;i<subjectdata.get(position).getDirectors().size();i++){
            sb_director.append(subjectdata.get(position).getDirectors().get(i).getName());
            if (i==subjectdata.get(position).getDirectors().size()-1){
        ...
        for (int y=0;y<subjectdata.get(position).getCasts().size();y++){
            sb_cast.append(subjectdata.get(position).getCasts().get(y).getName());
            if (y==subjectdata.get(position).getCasts().size()-1){
        ...
        String pinfen=String.valueOf(subjectdata.get(position).getRating().getAverage());
        holder.video_pingfen.setText("评分："+pinfen);
        StringBuffer sb_genere=new StringBuffer();
        String[] arr_genere=subjectdata.get(position).getGenres();
        ...
        Glide.with(context).load(subjectdata.get(position).getImages().getSmall()).into(holder.iv_videoavator);

        String videoid=subjectdata.get(position).getId();
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                bundle.putString("photourl",subjectdata.get(position).getImages().getSmall());
